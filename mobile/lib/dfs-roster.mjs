...
export function rosterKey(sport, platform) {
  return `${String(sport || "").toUpperCase()}|${String(platform || "draftkings").toLowerCase()}`;
}

export function getRoster(sport, platform) {
  return TEMPLATES[rosterKey(sport, platform)] || null;
}

export function slotLabel(slot, roster) {
...
export function normalizePlayerPos(raw) {
  let value = raw;
  ...
  const text = String(value || "")
    ...
    .replace("D/ST", "DST")
    .replace("D-ST", "DST");
  return text.split(/[/,]/).map((s) => s.trim()).filter(Boolean).map((p) => {
...
export function slotEligible(pos, slot, roster) {
  const positions = new Set(normalizePlayerPos(pos));
  let slotN = String(slot || "").toUpperCase();
...
export function averageRemainingPerPlayer(remainingSalary, remainingSlots) {
